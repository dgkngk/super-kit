...
export async function validateSpecConsistency(projectPath = '.') {
    ...
    try {
        ...
        for (const spec of specs) {
            if (!spec.isDirectory() || spec.name === 'templates')
                continue;
...
export async function completePlan(planFile, force = false, projectPath = '.') {
...
export async function validateArchitecture(projectPath = '.') {
    ...
    let fail = false;
    let out = '';
    if (actSkills !== expSkills) {
        out += `❌ Skills mismatch: Doc says ${expSkills}, Found ${actSkills}\n`;
        fail = true;
    }
...
export async function syncSpec(specDir, projectPath = '.') {
    // simplified implementation
    return `✓ Spec synchronized from spec.yaml in ${specDir}`;
}
export async function updateSpecPhase(specName, phaseNum, status, projectPath = '.') {
