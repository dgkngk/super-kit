...
export async function generateChangelog(projectPath = '.') {
    const typeMap = {
...
export async function validateChangelog(projectPath = '.') {
    const changelogPath = path.join(projectPath, 'CHANGELOG.md');
    let content = '';
    try {
        content = await fs.readFile(changelogPath, 'utf8');
...
export async function archiveCompleted(projectPath = '.', apply = false) {
    let archivedCount = 0;
    let skippedCount = 0;
    ...
    const doArchive = async (fileDestTuple) => {
    ...
    try {
        ...
        for (const file of todos) {
            if (!file.endsWith('.md') || file === 'todo-template.md' || file === 'archive')
                continue;
    ...
    try {
        ...
        for (const file of plans) {
            if (!file.endsWith('.md') || file === 'README.md' || file === 'archive')
                continue;
...
export async function prePushHousekeeping(projectPath = '.', fix = false) {
