...
export async function logWorkflow(workflow: string, session: string = '', projectPath: string = '.') {
    const timestamp = new Date().toISOString().replace(/\.[0-9]{3}Z$/, 'Z');
    if (!session) {
...
export async function rotateLogs(projectPath: string = '.', retentionDays: number = 90) {
    const logDir = getLogDir(projectPath);

    try {
        const stats = await fs.stat(logDir);
        if (!stats.isDirectory()) return 'Log directory does not exist.';
    } catch {
