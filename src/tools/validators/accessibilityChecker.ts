...
export async function findHtmlFiles(projectPath: string): Promise<string[]> {
    let files: string[] = [];
...
export async function checkAccessibility(filePath: string): Promise<string[]> {
    const issues: string[] = [];
    try {
        ...
        if (lowerContent.includes('tabindex=')) {
            if (!lowerContent.includes('tabindex="-1"') && !lowerContent.includes('tabindex="0"')) {
                ...
                if (positive) issues.push("Avoid positive tabIndex values");
            }
        }
...
export async function runAccessibilityChecker(projectPath: string = "."): Promise<{ passed: boolean, report: string }> {
    ...
    if (allIssues.length > 0) {
        for (const item of allIssues.slice(0, 10)) {
            report += `\n${item.file}:\n`;
            for (const issue of item.issues) report += `  - ${issue}\n`;
        }
        if (allIssues.length > 10) report += `\n... and ${allIssues.length - 10} more files with issues\n`;
    } else {
