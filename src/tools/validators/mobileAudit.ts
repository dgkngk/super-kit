...
class MobileAuditor {
    ...
    async auditFile(filePath: string) {
        try {
            ...
            if (!isReactNative && !isFlutter) return;
            ...
            for (const match of smallSizes) {
                if (parseInt(match[1]) < 44) this.issues.push(`[Touch Target] ${filename}: size ${match[1]}px < 44px`);
            }

            const smallGaps = [...content.matchAll(/(?:margin|gap):\s*([0-7])\s*(?:px|dp)/g)];
            for (const match of smallGaps) {
                if (parseInt(match[1]) < 8) this.warnings.push(`[Touch Spacing] ${filename}: gap ${match[1]}px < 8px`);
            }
            ...
            if (hasPrimaryBtn && !hasBottomObj) this.warnings.push(`[Thumb Zone] ${filename}: Primary CTA may not be at bottom`);
            ...
            if (isReactNative && /FlatList/.test(content)) {
                if (!/keyExtractor/.test(content)) this.issues.push(`[Performance CRITICAL] ${filename}: FlatList without keyExtractor`);
                if (/key=\{.*index.*?\}|key:\s*index/.test(content)) this.issues.push(`[Performance CRITICAL] ${filename}: Using index as key`);
            }

            if (isReactNative && /Animated\./.test(content)) {
                if (/useNativeDriver:\s*false/.test(content)) this.warnings.push(`[Performance] ${filename}: Animation.useNativeDriver is false`);
                if (!/useNativeDriver/.test(content)) this.warnings.push(`[Performance] ${filename}: Animated without useNativeDriver`);
            }
            ...
            if (consoleCount > 5) this.warnings.push(`[Performance] ${filename}: ${consoleCount} console statements logs`);

            if (isReactNative) {
                const inlines = (content.match(/(?:onPress|onPressIn|onPressOut|renderItem):\s*\([^)]*\)\s*=>/g) || []).length;
                if (inlines > 3) this.warnings.push(`[Performance] ${filename}: ${inlines} inline functions in props`);
            }
            ...
            if (tabs > 5) this.warnings.push(`[Navigation] ${filename}: ${tabs} tab items (max 5)`);
            ...
            for (const lh of lineHeights) {
                if (parseFloat(lh[1]) > 1.8) this.warnings.push(`[Typography] ${filename}: lineHeight > 1.8`);
            }

            const fontSizes = [...content.matchAll(/fontSize:\s*([\d.]+)/g)];
            for (const fs of fontSizes) {
                const size = parseFloat(fs[1]);
                if (size < 12) this.warnings.push(`[Typography] ${filename}: fontSize < 12px`);
                else if (size > 32) this.warnings.push(`[Typography] ${filename}: fontSize > 32px`);
            }
    ...
    async auditDirectory(dir: string) {
        try {
            ...
            for (const item of items) {
                if (['node_modules', '.git', 'dist', 'build', '.next', 'ios', 'android', '.idea'].includes(item.name)) continue;
...
export async function runMobileAudit(projectPath: string = "."): Promise<{ passed: boolean, report: string }> {
