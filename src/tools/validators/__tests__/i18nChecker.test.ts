...
import { checkLocaleCompleteness, checkHardcodedStrings } from '../i18nChecker.js';
...
describe('i18nChecker', () => {
    ...
    describe('checkLocaleCompleteness', () => {
        it('should detect missing translation keys in other locales', async () => {
            ...
            vi.mocked(fs.readFile).mockImplementation(async (file: string | any) => {
                if (file.includes('en')) return JSON.stringify({ hello: 'World', nested: { a: 1, b: 2 } });
                if (file.includes('fr')) return JSON.stringify({ hello: 'Monde', nested: { a: 1 } });
                return "{}";
            });
