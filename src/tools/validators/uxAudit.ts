...
class UXAuditor {
    ...
    async auditFile(filePath: string) {
        try {
            ...
            if (navItems > 7) this.issues.push(`[Hick's Law] ${filename}: ${navItems} nav items (Max 7)`);
            if (/height:\s*([0-3]\d)px/.test(content) || /h-[1-9]\b|h-10\b/.test(content)) this.warnings.push(`[Fitts' Law] ${filename}: Small targets (< 44px)`);
            if (complexElements > 7 && !/step|wizard|stage/i.test(content)) this.warnings.push(`[Miller's Law] ${filename}: Complex form (${complexElements} fields)`);
            if (lowerC.includes('button') && !/primary|bg-primary|Button.*primary|variant=["']primary/i.test(content)) this.warnings.push(`[Von Restorff] ${filename}: No primary CTA`);
            ...
            if (socialProof.length > 0) this.passed_count++;
            else if (hasLongText) this.warnings.push(`[Trust] ${filename}: No social proof detected.`);
            ...
            if (manyColors && manyBorders) this.warnings.push(`[Cognitive Load] ${filename}: High visual noise detected.`);
            ...
            if (googleFonts.length > 3) this.issues.push(`[Typography] ${filename}: >3 font families detected.`);
            ...
            if (/<h[1-6]|text-(?:xl|2xl|3xl|4xl|5xl|6xl)/i.test(content)) {
                ...
                for (const m of lhMatches) {
                    if (parseFloat(m[1]) > 1.5) this.warnings.push(`[Typography] ${filename}: Heading has line-height > 1.3.`);
                }
            }
            ...
            if (/@keyframes|transition:/.test(content) && /width|height|top|left|right|bottom|margin|padding/.test(content)) {
                ...
                if (!/prefers-reduced-motion/.test(content)) this.warnings.push(`[Accessibility] ${filename}: Animations found without prefers-reduced-motion check`);
            }
            ...
            if (gradients > 5) this.warnings.push(`[Visual] ${filename}: Many gradients detected (${gradients}).`);
            else if (hasHero && !/background:|bg-/.test(content)) this.warnings.push(`[Visual] ${filename}: Hero section without visual interest.`);

            if ((content.match(/border:/g) || []).length > 8) this.warnings.push(`[Visual] ${filename}: Many border declarations`);
            ...
            if (/color:\s*#000000|#000\b/.test(content)) this.warnings.push(`[Color] ${filename}: Pure black (#000000) detected.`);
            ...
            for (const d of durations) {
                ...
                if (ms < 50) this.warnings.push(`[Animation] ${filename}: Very fast animation (${d[1]}${d[2]}).`);
                else if (ms > 1000 && lowerC.includes('transition')) this.warnings.push(`[Animation] ${filename}: Long transition.`);
            }

            if (/ease-in\s+.*entry|fade-in.*ease-in/.test(content)) this.warnings.push(`[Animation] ${filename}: Entry animation with ease-in.`);
            if (/ease-out\s+.*exit|fade-out.*ease-out/.test(content)) this.warnings.push(`[Animation] ${filename}: Exit animation with ease-out.`);
            ...
            if (/<img(?![^>]*alt=)[^>]*>/.test(content)) this.issues.push(`[Accessibility] ${filename}: Missing img alt text`);

        } catch { }
    }

    async auditDirectory(dir: string) {
        try {
...
export async function runUxAudit(projectPath: string = "."): Promise<{ passed: boolean, report: string }> {
    ...
    if (res.issues.length > 0) {
        ...
        for (const i of res.issues.slice(0, 10)) report += `  - ${i}\n`;
    }
    if (res.warnings.length > 0) {
        report += `[*] WARNINGS (${res.warnings.length}):\n`;
        for (const w of res.warnings.slice(0, 15)) report += `  - ${w}\n`;
    }
