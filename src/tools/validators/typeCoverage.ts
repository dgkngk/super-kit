...
export interface CoverageResult {
    ...
    stats: {
        any_count: number;
        untyped_functions: number;
        total_functions: number;
        typed_functions?: number;
    };
}

async function getFiles(dir: string, extensionRegex: RegExp, excludeList: string[]): Promise<string[]> {
    let files: string[] = [];
    try {
        const items = await fs.readdir(dir, { withFileTypes: true });
        for (const item of items) {
            const fullPath = path.join(dir, item.name);
            if (excludeList.some(ex => fullPath.includes(ex))) continue;
...
export async function checkTypescriptCoverage(projectPath: string): Promise<CoverageResult> {
    const passed: string[] = [];
    const issues: string[] = [];
    ...
    for (const filePath of tsFiles.slice(0, 30)) {
        try {
            ...
            if (anyMatches) stats.any_count += anyMatches.length;

            // untyped functions
            const untypedFuncs = content.match(/function\s+\w+\s*\([^)]*\)\s*{/g);
            if (untypedFuncs) stats.untyped_functions += untypedFuncs.length;

            const untypedArrows = content.match(/=\s*\([^:)]*\)\s*=>/g);
            if (untypedArrows) stats.untyped_functions += untypedArrows.length;

            // typed functions
            let typedCount = 0;
            const typedFuncs = content.match(/function\s+\w+\s*\([^)]*\)\s*:\s*\w+/g);
            if (typedFuncs) typedCount += typedFuncs.length;

            const typedArrows = content.match(/:\s*\([^)]*\)\s*=>\s*\w+/g);
            if (typedArrows) typedCount += typedArrows.length;

            stats.total_functions += (typedCount + (untypedFuncs?.length || 0) + (untypedArrows?.length || 0));
        } catch { }
    }

    if (stats.any_count === 0) passed.push("[OK] No 'any' types found");
    else if (stats.any_count <= 5) issues.push(`[!] ${stats.any_count} 'any' types found (acceptable)`);
    else issues.push(`[X] ${stats.any_count} 'any' types found (too many)`);

    if (stats.total_functions > 0) {
        const typedRatio = ((stats.total_functions - stats.untyped_functions) / stats.total_functions) * 100;
        if (typedRatio >= 80) passed.push(`[OK] Type coverage: ${typedRatio.toFixed(0)}%`);
        else if (typedRatio >= 50) issues.push(`[!] Type coverage: ${typedRatio.toFixed(0)}% (improve)`);
        else issues.push(`[X] Type coverage: ${typedRatio.toFixed(0)}% (too low)`);
    }
...
export async function checkPythonCoverage(projectPath: string): Promise<CoverageResult> {
    const passed: string[] = [];
    const issues: string[] = [];
    ...
    if (stats.total_functions > 0) {
        ...
        if (typedRatio >= 70) passed.push(`[OK] Type hints coverage: ${typedRatio.toFixed(0)}%`);
        else if (typedRatio >= 40) issues.push(`[!] Type hints coverage: ${typedRatio.toFixed(0)}%`);
        else issues.push(`[X] Type hints coverage: ${typedRatio.toFixed(0)}% (add type hints)`);
    }

    if (stats.any_count === 0) passed.push("[OK] No 'Any' types found");
    else if (stats.any_count <= 3) issues.push(`[!] ${stats.any_count} 'Any' types found`);
    else issues.push(`[X] ${stats.any_count} 'Any' types found`);
...
export async function runTypeCoverage(projectPath: string = "."): Promise<{ passed: boolean, report: string }> {
    ...
    for (const res of results) {
        ...
        for (const p of res.passed) report += `  ${p}\n`;
        for (const i of res.issues) {
            report += `  ${i}\n`;
            if (i.startsWith("[X]")) criticalIssues++;
        }
    }
