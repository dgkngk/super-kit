...
export async function compoundSearch(terms: string[], projectPath: string = '.'): Promise<string> {
...
export async function updateSolutionRef(files: string[], projectPath: string = '.'): Promise<string> {
...
export async function validateCompound(projectPath: string = '.'): Promise<string> {
    let exitCode = 0;
    let output = "🔍 Validating Compound System...\n";

    async function countUnchecked(filePath: string): Promise<number> {
        try {
...
export async function auditStateDrift(projectPath: string = '.', fix: boolean = false): Promise<string> {
    let driftCount = 0;
    let fixCount = 0;
    let output = "🔍 Auditing Lifecycle Document State Consistency...\n\n";

    async function checkFile(p: string, docType: 'plan' | 'todo') {
        try {
    ...
    try {
        ...
        for (const f of plans) if (f.endsWith('.md')) await checkFile(path.join(projectPath, 'plans', f), 'plan');
    } catch { }

    try {
        const todos = await fs.readdir(path.join(projectPath, 'todos'));
        for (const f of todos) if (f.endsWith('.md') && !f.includes('template')) await checkFile(path.join(projectPath, 'todos', f), 'todo');
    } catch { }
    ...
    if (driftCount === 0 && fixCount === 0) {
        ...
        if (driftCount > 0) output += `⚠️  Remaining drift in ${driftCount} documents.`;
    } else {
...
export async function suggestSkills(projectPath: string = '.'): Promise<string> {
    let output = "🔍 Skill Opportunity Audit...\nAnalyzing knowledge base for patterns...\n\n";
    const tagCounts: Record<string, number> = {};

    async function walkSolveDocs(dir: string) {
        try {
...
export async function compoundHealth(projectPath: string = '.'): Promise<string> {
    const reportList: string[] = ["🏥 COMPOUND SYSTEM HEALTH", "========================", ""];
    ...
    try {
        const walk = async (dir: string) => {
            const files = await fs.readdir(dir, { withFileTypes: true });
            for (const file of files) {
                if (file.isDirectory()) await walk(path.join(dir, file.name));
                else if (file.name.endsWith('.md')) solCount++;
            }
        };
...
export async function compoundDashboard(projectPath: string = '.'): Promise<string> {
    return compoundHealth(projectPath); // Alias for now as they provide similar info
}

export async function compoundMetrics(projectPath: string = '.', force: boolean = false): Promise<string> {
