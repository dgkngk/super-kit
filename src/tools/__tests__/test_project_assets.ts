...
async function create_agent_file(agents_dir: string, name: string, content = `# ${name}`) {
  await fs.mkdir(agents_dir, { recursive: true });
  await fs.writeFile(path.join(agents_dir, `${name}.md`), content, 'utf-8');
}

async function create_skill_file(
  skills_dir: string,
  category: string,
  skill_name: string,
  content = `# ${skill_name} SKILL`,
) {
...
async function create_workflow_file(
...
describe('ProjectAssets', () => {
  ...
  describe('load_project_agent_file', () => {
    ...
    it('throws a meaningful error when agent file does not exist', async () => {
      await expect(load_project_agent_file('nonexistent', temp_dir)).rejects.toThrow(
        'Project agent not found: "nonexistent"',
      );
    });

    it('rejects path traversal via ../ in agent name', async () => {
      await expect(
        load_project_agent_file('../../../etc/passwd', temp_dir),
      ).rejects.toThrow(/invalid path components|traversal/i);
    });

    it('rejects an absolute path as agent name', async () => {
      await expect(
        load_project_agent_file('/etc/passwd', temp_dir),
      ).rejects.toThrow(/absolute path|invalid/i);
    });

    it('throws when agentName is an empty string', async () => {
      await expect(load_project_agent_file('', temp_dir)).rejects.toThrow(
        /non-empty string/i,
      );
    });
    ...
    it('loads correctly when projectPath is omitted (uses cwd as base)', async () => {
      ...
      await expect(load_project_agent_file('any-agent')).rejects.toThrow(
        'Project agent not found: "any-agent"',
      );
    });
  });
  ...
  describe('load_project_skill_file', () => {
    ...
    it('throws a meaningful error when SKILL.md does not exist', async () => {
      await expect(
        load_project_skill_file('tech', 'nonexistent-skill', temp_dir),
      ).rejects.toThrow('Project skill not found');
    });

    it('rejects path traversal in category', async () => {
      await expect(
        load_project_skill_file('../../etc', 'passwd', temp_dir),
      ).rejects.toThrow(/invalid path components|traversal/i);
    });

    it('rejects path traversal in skillName', async () => {
      await expect(
        load_project_skill_file('tech', '../../../etc/passwd', temp_dir),
      ).rejects.toThrow(/invalid path components|traversal/i);
    });
    ...
    it('throws when category is an empty string', async () => {
      await expect(load_project_skill_file('', 'some-skill', temp_dir)).rejects.toThrow(
        /non-empty string/i,
      );
    });

    it('throws when skillName is an empty string', async () => {
      await expect(load_project_skill_file('tech', '', temp_dir)).rejects.toThrow(
        /non-empty string/i,
      );
    });
  ...
  describe('load_project_workflow_file', () => {
    ...
    it('throws a meaningful error when workflow file does not exist', async () => {
      await expect(
        load_project_workflow_file('nonexistent-workflow', temp_dir),
      ).rejects.toThrow('Project workflow not found: "nonexistent-workflow"');
    });

    it('rejects path traversal via ../ in workflow name', async () => {
      await expect(
        load_project_workflow_file('../../../etc/passwd', temp_dir),
      ).rejects.toThrow(/invalid path components|traversal/i);
    });
    ...
    it('throws when workflowName is an empty string', async () => {
      await expect(load_project_workflow_file('', temp_dir)).rejects.toThrow(
        /non-empty string/i,
      );
    });

    it('loads correctly when projectPath is omitted', async () => {
      await expect(load_project_workflow_file('any-workflow')).rejects.toThrow(
        'Project workflow not found: "any-workflow"',
      );
    });
  });
