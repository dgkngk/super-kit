...
export async function bootstrapFolderDocs(folder: string, projectPath: string = '.'): Promise<string> {
...
export async function checkDocsFreshness(skipDocs: boolean = false, projectPath: string = '.'): Promise<string> {
    if (skipDocs) return "⏭️  Skipping documentation freshness check (--skip-docs)";
...
export async function discoverUndocumentedFolders(projectPath: string = '.'): Promise<string> {
    ...
    async function scanDir(dir: string, depth: number) {
        ...
        try {
            ...
            for (const entry of entries) {
                if (exclusions.some(ex => entry.name.includes(ex))) continue;

                if (entry.isDirectory()) {
                    await scanDir(path.join(dir, entry.name), depth + 1);
                } else if (entry.isFile()) {
                    if (entry.name === 'README.md') hasReadme = true;
                    if (/\.(ts|tsx|py|sh)$/.test(entry.name)) hasSource = true;
                }
            }
    ...
    if (undocumented.length > 0) {
        ...
        for (const folder of undocumented) out += `   - ${folder}\n`;
        return out;
    }
...
export async function validateFolderDocs(strict: boolean = false, targetFolders: string[] = [], projectPath: string = '.'): Promise<string> {
    ...
    for (const folder of foldersToCheck) {
        ...
        const missing = [];
        if (!content.includes("## Purpose")) missing.push("Purpose");
        if (!/## (Key )?Components/.test(content)) missing.push("Components");
        if (!content.includes("## Component Details")) missing.push("Component Details");
        if (!content.includes("## Changelog")) missing.push("Changelog");
